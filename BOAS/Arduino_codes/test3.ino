...
void UI()
{
  ...
  {
    Serial.println("<-------------------------------->");
    Serial.println(" |   1. Verify ID               |");
    Serial.println(" |   2. Enroll new Student      |");
    Serial.println(" |   3. Delete ALL ID           |");
    Serial.println(" |   4. UI                      |");
    Serial.println("<-------------------------------->");
  }
  else
...
void loop()
{
   ...
   if(Serial.available() > 0)               // If there is any serial data command packet available on the bluetooth
   {
     
         commandval = Serial.parseInt();       // Get the integer form of the command packet string
         rec_ID = commandval-((commandval/1000)*1000);  // Get the received value's ID portion
         switch(commandval)
         {
        case 1:             // If the thousands digit is 1 -> marking attendence condition
         {
             blink_LED();
        ...
        case 2:              // Enroll Command
         ...
         default: Serial.println("enter any value");
      break;
         }
         while(Serial.available() > 0)
         {Serial.read();}  //Flush out if there is anything else in the Serial port of bluetooth module
...
void get_str() 
{
     if (unique_id < 10)
      {
            sendstring = String(unique_id); //The unique_id must be a string of three digits. It ranges from 0-199
      }
      else
      {
            if (unique_id < 100)
      ...
      Serial.println(sendstring);    // Send the string to the calling device. This string is of the unique ID to be sent. The string is composed of 3 bytes
      Serial3.println(sendstring);   // Check on the hard wired serial port for the string that is sent
      delay(500);
}
